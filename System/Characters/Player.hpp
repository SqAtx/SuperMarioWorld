...
{
	public:
		Player(EventEngine *_eventEngine, std::string _name, sf::Vector2f _coord, State _state = UNKNOWN);
		Player(EventEngine *_eventEngine, std::string _name, float _x, float _y, State _state = UNKNOWN);
		~Player();
